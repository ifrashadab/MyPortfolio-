...
export default function HeroSection() {
  ...
  const phrases = ["Electrical Engineer", "AI & Cybersecurity Enthusiast", "Innovator"]

  // NEW: State to hold the bubble styles
  const [bubbles, setBubbles] = useState<React.CSSProperties[]>([])

  // This useEffect for typing is perfect, leave it as is
  useEffect(() => {
    let charIndex = 0
  ...
  // NEW: useEffect to generate bubbles ONLY on the client after mount
  useEffect(() => {
  ...
  return (
    ...
    >
      <div className="absolute inset-0">
        <Image src="/profile.jpg" alt="Background" fill className="w-full h-full object-cover" priority />
        <div className="absolute inset-0 bg-gradient-to-br from-background/95 via-background/90 to-background/95" />
      </div>

      {/* MODIFIED: Map over the 'bubbles' state variable */}
      <div className="absolute inset-0 opacity-20">
        {bubbles.map((style, i) => (
          <div
            key={i}
            className="absolute rounded-full bg-gradient-to-r from-primary/20 to-accent/20"
            style={style} // Apply the style from state
          />
        ))}
      </div>
